...
def create_tab_accueil(notebook):
    ...
    button_random = tk.Button(
        tab,
        text="Affiche une recette aléatoire !",
        style="primary",
        command=show_random_recipe,
    )
    ...
    global recipe_text
    recipe_text = tk.Text(
        text_frame, height=20, width=80, state="disabled"
    )  # state = lecture seule
...
def show_random_recipe():
    ...
    if recipe:
        recipe_text.config(
            state="normal"
        )  # Réactiver pour insérer du texte // disabled
...
def create_tab_entree(notebook):
    ...
    for recipe in entree_recipes:
        entree_listbox.insert(
            tk.END, recipe[1]
        )  # Le 1 permet de montrer le nom (1ere pos)
...
{recipe[5]}""",
    ...
    delete_button = tk.Button(
        ...
        command=lambda: button_delete(
            "entree", entree_listbox, entree_text, entree_recipes
        ),
        bootstyle="danger",
    )
...
{recipe[5]}""",
    ...
    delete_button = tk.Button(
        plat_frame,
        text="Supprimer la recette",
        command=lambda: button_delete("plat", plat_listbox, plat_text, plat_recipes),
        bootstyle="danger",
    )
    delete_button.pack(pady=5, padx=3)
...
{recipe[5]}""",
    ...
    delete_button = tk.Button(
        ...
        text="Supprimer la recette",
        command=lambda: button_delete(
            "dessert", dessert_listbox, dessert_text, dessert_recipes
        ),
        bootstyle="danger",
    )
    delete_button.pack(pady=5, padx=3)
...
def create_tab_ajout(notebook):
    ...
    )  # on initie à entrée, on evite les erreurs de mauvaise table

    table_combo = tk.Combobox(
        tab4,
    ...
    def add_recipe_button():
        ...
        try:
            ...
            if any(char.isdigit() for char in name_input):
                raise ValueError(
                    "Le nom de la recette ne doit pas contenir de chiffres."
                )

            # Validation du champ 'nombre de personnes'
            people_input = (
                ppl_entry.get().strip()
            )  # Récupérer directement depuis l'Entry
            if not people_input:
            ...
            try:
                ...
                if "invalid literal" in str(e):
                    raise ValueError(
                        "Le nombre de personnes doit être un nombre entier valide."
                    )
            ...
            try:
                ...
                if "invalid literal" in str(e):
                    raise ValueError(
                        "Le temps de préparation doit être un nombre entier valide."
                    )
            ...
            if not ins_input:
                raise ValueError(
                    "Remplissez un minimum les instructions pour faire la recette."
                )

            motor.add_recipe(
                table_var.get(), name_input, people_val, time_val, ing_input, ins_input
            )
            ...
            Messagebox.show_info(
                title="Succès", message="Recette ajoutée avec succès !"
            )
    ...
    add_button = tk.Button(
        tab4, text="Ajouter la recette", command=add_recipe_button, bootstyle="success"
    )
