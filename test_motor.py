...
def test_add_recipe(database):
    motor2.add_recipe(database, "dessert", "Tarte aux fraises", 8, 45, "Lait, oeufs, fraise", "Faire la pâte puis la crème et tout assembler")
...
def test_get_recipes_by_category(database):
    motor2.add_recipe(database, "dessert", "Flan", 4, 75, "Oeuf, sucre, lait", "Tout mélanger puis cuire")
    motor2.add_recipe(database, "plat", "Boeuf Bourguignon", 4, 50, "Boeuf, légumes, vin", "Cuire le boeuf et les légumes en sauce")
...
def test_get_random_recipe(database):
    motor2.add_recipe(database, "plat", "Boeuf Bourguignon", 4, 50, "Boeuf, légumes, vin", "Cuire le boeuf")
    motor2.add_recipe(database, "entree", "Pâtes Pesto", 2, 30, "Pâtes, pesto, tomate, mozzarella", "Cuire et mélanger")
    motor2.add_recipe(database, "dessert", "Flan", 4, 75, "Oeuf, sucre, lait", "Tout mélanger puis cuire")
...
def test_add_recipe_invalid_table(database):
    with pytest.raises(ValueError, match="Table inconnue"):
        motor2.add_recipe(database, "boisson", "Coca", 1, 0, "Eau, sucre", "Verser dans un verre")
...
def test_multiple_recipes(database):
    motor2.add_recipe(database, "entree", "Salade", 2, 10, "Laitue, vinaigrette", "Mélanger")
